...
系统间的调用，不可避免存在一些重试，例如超时重试，或者手工的重试，必须要保证重试不会产生负面影响，或者产生脏数据，甚至是影响正常数据的正确性。

### 实践要点

重试一般都会有不同的原因，比如：

- 正常重试，例如超时重试、错误重试、后台手工操作重试等等
- 恶意重试，一般是攻击

针对不同原因可以有不同的方案，像恶意攻击，直接拦截此类请求即可，这个属于安全防范的范畴，不在本节的讨论范围内，让我们来看看如何保证正常的重试。

重试时有两种情况——第一，上次请求仍在执行中；第二，上次请求已经完成。对于前者，建议不要重复执行，一来避免重复计算浪费资源，二来可以避免同时执行时发生未知问题，比如线程间相互覆盖已写入的值；对于后者，在执行前可以先判断下待操作资源的状态，如果已在终态，则可以直接跳过。

>*注意：判断待操作资源状态时，可以加锁已防止并发问题，比如两个请求同时获得了可执行状态，单进程的应用可以用内存锁，集群部署的应用可以用分布式锁，或者简单地通过数据库锁加以保证。对于那些要求不高的应用，也可省略这一步骤。*

**通过异步消息进行通信的系统中，除了要能处理消息重发，还要能够正常处理消息乱序的情况。** 所谓乱序，即系统A先发出消息1再发出消息2，两者是有序的，但是由于网络或系统原因，导致系统B先收到消息2，随后再收到消息1。无论何种消息到达顺序，最终系统中资源的状态都必须最终一致。

提供REST服务的系统，请注意使用正确的HTTP动词，根据规范，每个HTTP动词都有自己的幂等性特征。

- GET/HEAD/OPTIONS/TRACE是幂等的
- POST/PUT/DELETE是非幂等的

因此，使用POST来处理所有的请求，是一个错误的做法；同样的，在GET方法中修改资源的状态，也是一个错误的做法。

### 案例说明

比如，请求执行一个任务，在开始执行现可以先判断Task.status，避免同时执行相同任务或者重复执行相同任务：

1. INIT状态，说明任务准备就绪，可以正常执行。
2. PROCESSING状态，说明正在执行，本次请求结束。
3. SUCCESS或FAILED状态，说明任务已执行完毕，本次请求结束；当然FAILED状态也可作为失败重试，再执行一次。

接下来，以一个交易系统为例，再让我们来看看消息乱序的问题。假设，交易系统A在交易创建、付款、收货环节都会向其他系统发送消息，而周边系统B会接受这些消息，并保存下来，创建、付款和收货消息分别以A1、A2和A3表示。由于网络原因，系统B接受消息的顺序是A2->A3->A1，显然不能按照接收顺序保存数据，否则该条交易的最终状态会是“已创建”，而不是“已收货”。

为此，可以在接收消息时，通过代码判断消息负载中的交易数据状态，以此判断是否需要更新数据。另外，考虑到消息中的数据可能不完整，例如每次只发送变更部分，也可以在接收到消息时，每次都从交易系统A中查询最新的数据进行保存，消息仅作触发使用。

## Rule 7. 系统间的交互尽量采用异步方式

### 目的描述

虽然同步调用方便直观，但是同步调用不利于系统的扩展，而且在业务处理的关键路径中多增加一个环节就会多一分失败的可能，应该尽可能地使用异步的方式进行系统交互，将不必要的同步调用改为异步后，也可降低整体的延时。

### 实践要点

一般异步通信可以使用消息中间件来实现，比如RabbitMQ。对于不需要关心结果的中间操作，在发送消息后可以直接进行后续操作；而那些需要等待结果的操作，可以在发送消息后，等待回调，如果无法支持回调，那么可以采用轮询的方式去对方系统查询执行结果。

消息的发布订阅模式可以用于很多场景之中，例如系统A、系统B都关心系统C中的某个业务执行，当系统C收到业务请求后在处理自己逻辑的同时，可以发布一个消息，关心该消息的系统A和B都订阅了该消息，收到消息后可以各自进行后续处理。如此一来既不影响系统C的业务，也能完成各自的任务。

采用REST服务，也能实现异步交互。具体步骤如下：

1. 创建一个异步任务资源，即向/tasks发送一个POST请求，包含必要参数和回调地址（如果不用回调则无需包含该参数）
2. 资源创建后直接返回包含该异步任务资源URL的201 Created，调用方继续后续操作，而具体异步任务在被调用方执行
3. 异步任务执行完毕后修改异步任务资源状态，如果需要回调，则进行回调，不需要回调的等待调用方查询状态，或者直接结束
4. 如果调用方需要关心结果，又不做回调，可以轮询第二步返回的URL，知道得到结果，一般结果中直接带有想要的内容，或者是返回一个303 See Other

### 案例说明

以交易为例，一笔交易在付款后需要进行账务的操作，除了记录主账务系统A的数据，供客户和商户分别查询准实时账务数据的系统B和C也需要同步一份数据，显然在主账务操作的同时，顺序或并行地由系统A同步调用B和C落地数据是不合适的，B和C中任意一方的失败都不应该影响主账务数据的正常保存。此时可以由系统A在保存完本地数据后，发送消息，而B和C各自订阅A的消息。如果系统B发生异常，消息可以积压在消息中间件中，待系统恢复后，再投递给系统B；也可以直接丢弃消息，系统B恢复后，通过自己的补偿机制恢复故障期间的数据。可见，**采用异步的交互方式可以在故障隔离方面发挥一定作用。**

以某网关系统为例，该系统负责调用外部的第三方服务，内部系统本身的吞吐量在10000TPM，响应时间10ms，而第三方服务不仅吞吐量不高，仅5000TPM，而且耗时较长，为100ms。如果内部系统在处理业务时同步调用网关，不仅可能影响自身的吞吐量，而且还大大增加了响应时间。可以将对网关的调用从同步调用中剥离出来，采用异步的方式进行调用，在第三方系统响应请求后回调内部业务系统。